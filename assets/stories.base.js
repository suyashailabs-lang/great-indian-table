...
(function(){
  ...
  style.textContent=`
    ...
    .hero-actions{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:42px}
    .hero-actions .hero-cta{margin-top:0}
    .hero-cta{position:relative;overflow:hidden;align-items:center;min-height:46px;padding:0 18px!important;border:1px solid rgba(255,255,255,.42)!important;border-radius:999px;background:rgba(11,16,15,.34);backdrop-filter:blur(14px) saturate(135%);-webkit-backdrop-filter:blur(14px) saturate(135%);box-shadow:0 10px 32px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.2);will-change:transform,filter,box-shadow;animation:gitEnterAttention 4.2s ease-in-out infinite}
    .hero-cta:before{content:"";position:absolute;top:-20%;bottom:-20%;left:-35%;width:26%;background:linear-gradient(110deg,transparent,rgba(255,255,255,.64),rgba(255,235,214,.22),transparent);filter:blur(1px);transform:skewX(-18deg);pointer-events:none;animation:gitEnterGlitter 4.2s ease-in-out infinite}
    .hero-cta:after{content:"";position:absolute;inset:-7px -9px;border:1px solid rgba(255,241,226,.28);border-radius:999px;box-shadow:0 0 0 0 rgba(239,154,126,0);opacity:0;pointer-events:none;animation:gitEnterRing 4.2s ease-in-out infinite}
    .hero-cta span{position:relative;z-index:1}
    @keyframes gitEnterGlitter{0%,57%,100%{left:-35%;opacity:0}63%{left:28%;opacity:.92}70%{left:112%;opacity:0}}
    @keyframes gitEnterAttention{0%,64%,100%{filter:brightness(1);transform:translateY(0);box-shadow:0 10px 32px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.2)}69%{filter:brightness(1.22);transform:translateY(-1px);box-shadow:0 12px 38px rgba(0,0,0,.32),0 0 22px rgba(239,154,126,.17),inset 0 1px 0 rgba(255,255,255,.28)}74%{filter:brightness(1);transform:translateY(0);box-shadow:0 10px 32px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.2)}}
    @keyframes gitEnterRing{0%,63%,100%{opacity:0;transform:scale(.9);box-shadow:0 0 0 0 rgba(239,154,126,0)}67%{opacity:1;transform:scale(1);box-shadow:0 0 0 6px rgba(239,154,126,.08),0 0 28px rgba(239,154,126,.18)}74%{opacity:0;transform:scale(1.08);box-shadow:0 0 0 14px rgba(239,154,126,0),0 0 38px rgba(239,154,126,0)}}
    .hero-info-btn{position:relative;width:auto;min-width:124px;height:46px;padding:0 17px;border:1px solid rgba(248,243,233,.48);border-radius:999px;display:inline-flex;align-items:center;justify-content:center;gap:8px;background:rgba(12,17,16,.30);backdrop-filter:blur(14px) saturate(125%);-webkit-backdrop-filter:blur(14px) saturate(125%);font:500 10px/1 var(--mono);letter-spacing:.11em;text-transform:uppercase;box-shadow:0 10px 30px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.16);transition:transform .25s ease,background .25s ease,border-color .25s ease,box-shadow .25s ease}
    .hero-info-btn .info-icon{width:20px;height:20px;border:1px solid currentColor;border-radius:50%;display:grid;place-items:center;font:600 12px/1 var(--body);text-transform:none;letter-spacing:0}
    .hero-info-btn:hover,.hero-info-btn:focus-visible{transform:translateY(-2px);background:rgba(18,24,23,.52);border-color:rgba(255,255,255,.82);box-shadow:0 12px 34px rgba(0,0,0,.32),inset 0 1px 0 rgba(255,255,255,.23);outline:none}
    ...
    /* About Us: editorial reading experience. */
    .git-about-backdrop{position:fixed;inset:0;z-index:200;background:radial-gradient(circle at 50% 15%,rgba(239,154,126,.08),transparent 38%),rgba(0,0,0,.72);backdrop-filter:blur(14px) saturate(115%);-webkit-backdrop-filter:blur(14px) saturate(115%);display:flex;align-items:center;justify-content:center;padding:28px;opacity:0;pointer-events:none;transition:opacity .28s ease}
    .git-about-backdrop.is-open{opacity:1;pointer-events:auto}
    .git-about-modal{position:relative;width:min(920px,94vw);max-height:min(84vh,860px);overflow:auto;background:linear-gradient(145deg,rgba(37,42,38,.96),rgba(15,20,19,.97));border:1px solid rgba(255,255,255,.18);border-radius:24px;box-shadow:0 36px 110px rgba(0,0,0,.58),inset 0 1px 0 rgba(255,255,255,.08);padding:30px 42px 30px;color:#f8f3e9;isolation:isolate}
    .git-about-modal:before{content:"";position:absolute;inset:0;z-index:-1;opacity:.34;border-radius:inherit;background:radial-gradient(circle at 12% 10%,rgba(239,154,126,.25),transparent 28%),radial-gradient(circle at 90% 85%,rgba(210,181,132,.12),transparent 34%),repeating-linear-gradient(0deg,rgba(255,255,255,.012) 0,rgba(255,255,255,.012) 1px,transparent 1px,transparent 5px);pointer-events:none}
    .git-about-modal:after{content:"";position:absolute;top:0;left:42px;right:42px;height:2px;background:linear-gradient(90deg,transparent,rgba(239,154,126,.72),rgba(248,243,233,.34),rgba(239,154,126,.72),transparent);opacity:.8}
    .git-about-close{position:sticky;float:right;top:0;width:36px;height:36px;border-radius:50%;display:grid;place-items:center;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.17);font-size:20px;cursor:pointer;z-index:2;transition:background .2s ease,transform .2s ease}
    .git-about-close:hover,.git-about-close:focus-visible{background:rgba(255,255,255,.15);transform:rotate(4deg);outline:none}
    .git-about-modal h3{margin:4px 54px 26px 0;font:700 clamp(1.45rem,2.7vw,2.25rem)/1.14 var(--display);letter-spacing:-.03em;color:#fff;text-shadow:0 3px 18px rgba(0,0,0,.36)}
    .git-about-modal h3:after{content:"";display:block;width:54px;height:3px;margin-top:16px;background:var(--accent);border-radius:3px;box-shadow:0 0 18px rgba(239,154,126,.28)}
    .git-about-modal p,.git-about-modal li{font:400 clamp(.95rem,1.23vw,1.06rem)/1.78 var(--body);color:rgba(248,243,233,.9)}
    .git-about-modal p{margin:0 0 17px;max-width:790px}
    .git-about-modal p:first-of-type{font-size:clamp(1.08rem,1.45vw,1.22rem);line-height:1.62;color:#fff;font-weight:500}
    .git-about-modal p:nth-of-type(3){margin-top:22px;padding:18px 20px 18px 22px;border-left:2px solid rgba(239,154,126,.72);background:rgba(255,255,255,.045);border-radius:0 12px 12px 0;box-shadow:inset 0 1px 0 rgba(255,255,255,.04)}
    .git-about-modal p:nth-of-type(4){color:rgba(248,243,233,.82)}
    .git-about-modal em{color:#f1ba98}
    .git-about-modal ul{margin:4px 0 20px;padding:0;list-style:none;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px 12px;max-width:790px}
    .git-about-modal li{margin:0;padding:12px 14px;border:1px solid rgba(255,255,255,.12);border-radius:12px;background:rgba(255,255,255,.045);box-shadow:inset 0 1px 0 rgba(255,255,255,.04);font-size:.94rem;line-height:1.55}
    .git-about-modal li:before{content:"•";color:var(--accent);margin-right:8px}
    .git-about-modal a{color:#f2b28e;text-decoration:underline;text-decoration-color:rgba(242,178,142,.52);text-underline-offset:3px;transition:color .2s ease,text-decoration-color .2s ease}
    .git-about-modal a:hover{color:#ffd1ba;text-decoration-color:#ffd1ba}
    .git-about-x{display:inline-flex;align-items:center;justify-content:center;gap:9px;margin:16px auto 0;padding:12px 18px;border-radius:999px;background:#050605;border:1px solid rgba(255,255,255,.2);color:#fff!important;text-decoration:none!important;font:500 10px var(--mono)!important;letter-spacing:.08em;text-transform:uppercase;box-shadow:0 8px 28px rgba(0,0,0,.3);transition:transform .2s ease,background .2s ease}
    .git-about-x:hover{transform:translateY(-2px);background:#0d100e}
    @media(max-width:800px){
      .hero-actions{margin-top:34px;gap:10px}.hero-info-btn{min-width:112px;height:42px;padding:0 14px;font-size:9px}.hero-info-btn .info-icon{width:18px;height:18px;font-size:11px}.hero-cta{min-height:42px;padding:0 15px!important}
      .hero-kicker{font-size:11px!important;line-height:1.5!important;padding:0 8px}
      ...
      .git-about-backdrop{padding:12px}.git-about-modal{max-height:90vh;padding:26px 20px 22px;border-radius:18px}.git-about-modal:after{left:20px;right:20px}.git-about-modal h3{font-size:1.35rem;margin-right:44px}.git-about-modal p,.git-about-modal li{font-size:.93rem;line-height:1.65}.git-about-modal p:first-of-type{font-size:1.02rem}.git-about-modal p:nth-of-type(3){padding:15px 15px 15px 17px}.git-about-modal ul{grid-template-columns:1fr;gap:8px}.git-about-x{margin-top:10px}
    }
