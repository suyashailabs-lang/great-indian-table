...
(function(){
  ...
  /* Warm the browser cache for every story's primary image. This fixes the lazy-thumbnail
     behaviour while keeping alternate images on demand. */
  const preloadStoryImages=()=>{
    const urls=[...new Set((window.STORIES||[]).map(s=>s.images?.[0]).filter(Boolean))];
    const load=()=>urls.forEach((src,i)=>{const img=new Image();img.decoding='async';img.fetchPriority=i<4?'high':'low';img.src=src});
    if('requestIdleCallback' in window)requestIdleCallback(load,{timeout:1200});else setTimeout(load,250);
  };
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',preloadStoryImages,{once:true});else preloadStoryImages();
